...
//! turns the result into [`start`](AudioPlayback::start) /
//! [`update`](AudioPlayback::update) / [`stop`](AudioPlayback::stop) calls on a
//! pluggable backend `P`. The backend — software (`rameau_software`) or a
//! real-time engine (`rameau_kira`) — owns resampling, pitch, mixing and output.
//!
